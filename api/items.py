...
def get(request):
    ...
    basic_tagged_items = __googlesheets.rand_tagged_values("Items", valid_tags=tags, limit=limit)
    
    final_items = []
    
    # get all the spells (randomized)!
    tagged_spells = __googlesheets.rand_tagged_values("Spells", valid_tags=tags, limit=limit)
    ...
    while len(basic_tagged_items) > 0 and len(final_items) < limit:
        full_item = basic_tagged_items.pop(random.randint(0, len(basic_tagged_items) - 1))
        
        if random.randint(0, 100) <= spellchance:
            full_item.append(tagged_spells.pop(random.randint(0, len(tagged_spells) - 1)))
